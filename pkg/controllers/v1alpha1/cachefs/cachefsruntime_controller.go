...
var (
	_ controllers.RuntimeReconcilerInterface = (*CacheFSRuntimeReconciler)(nil)
)
...
func (r *CacheFSRuntimeReconciler) Reconcile(context context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	ctx.Runtime = runtime
	ctx.EngineImpl = ddc.InferEngineImpl(runtime.Status, common.CacheFSEngineImpl)
...
func (r *CacheFSRuntimeReconciler) SetupWithManager(mgr ctrl.Manager, options controller.Options, eventDriven bool) error {
	if eventDriven {
		...
		return ctrl.NewControllerManagedBy(mgr).
			WithOptions(options).
			For(&datav1alpha1.CacheFSRuntime{}).
			Complete(r)
	}
}
