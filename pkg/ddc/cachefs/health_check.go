...
func (c *CacheFSEngine) CheckRuntimeHealthy() (err error) {
	// 1. Check the healthy of the workers
	err = c.checkWorkersHealthy()
	if err != nil {
		c.Log.Error(err, "The workers are not healthy")
		updateErr := c.UpdateDatasetStatus(data.FailedDatasetPhase)
		if updateErr != nil {
			c.Log.Error(updateErr, "Failed to update dataset")
		}
		return
	}

	// 2. Check the healthy of the fuse
	err = c.checkFuseHealthy()
	if err != nil {
		c.Log.Error(err, "The fuse is not healthy")
		updateErr := c.UpdateDatasetStatus(data.FailedDatasetPhase)
		if updateErr != nil {
			c.Log.Error(updateErr, "Failed to update dataset")
		}
	...
	updateErr := c.UpdateDatasetStatus(data.BoundDatasetPhase)
	if updateErr != nil {
		c.Log.Error(updateErr, "Failed to update dataset")
	}
...
func (c *CacheFSEngine) checkWorkersHealthy() (err error) {
	workerName := c.getWorkerName()

	// Check the status of workers
	workers, err := kubeclient.GetStatefulSet(c.Client, workerName, c.namespace)
	if err != nil {
		return err
	}
	...
	err = retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if workers.Status.ReadyReplicas == 0 && *workers.Spec.Replicas > 0 {
			...
			c.Log.Error(err, "the workers are not ready")
		} else {
			healthy = true
		...
		if !reflect.DeepEqual(runtime.Status, runtimeToUpdate.Status) {
			updateErr := c.Client.Status().Update(context.TODO(), runtimeToUpdate)
			if updateErr != nil {
				return updateErr
			}
		}
	...
	if err != nil {
		c.Log.Error(err, "Failed update runtime")
		return err
	}
...
func (c *CacheFSEngine) checkFuseHealthy() (err error) {
	fuseName := c.getFuseDaemonsetName()

	fuses, err := c.getDaemonset(fuseName, c.namespace)
	if err != nil {
		return err
	}
	...
	err = retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if fuses.Status.NumberUnavailable > 0 ||
			...
			c.Log.Error(err, "Failed to check the fuse healthy")
		} else {
			healthy = true
