...
func TestGetCacheInfoFromConfigmap(t *testing.T) {
	...
	wantCacheInfo := map[string]string{"mountpath": "/runtime-mnt/cachefs/default/cfsdemo/cachefs-fuse"}
	cacheinfo, _, err := GetCacheInfoFromConfigmap(fakeClient, dataSet.Name, dataSet.Namespace)
	if err != nil {
...
func Test_parseCacheInfoFromConfigMap(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotPorts, _, err := parseCacheInfoFromConfigMap(tt.args.configMap)
