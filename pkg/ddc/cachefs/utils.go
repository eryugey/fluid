...
func (c *CacheFSEngine) getFuseDaemonsetName() string {
	return c.name + "-fuse"
}

func (c *CacheFSEngine) getWorkerName() string {
	return c.name + "-worker"
}

func (c *CacheFSEngine) getMasterName() string {
	return c.name + "-master"
}
...
func (c *CacheFSEngine) parseCacheFSImage(forMaster bool, image, tag, imagePullPolicy string) (string, string, string, error) {
	defaultImage := common.CacheFSDefaultImage
	...
	if forMaster {
		defaultImage = common.CacheFSDefaultMasterImage
		imageFromEnv = ""
		imageTagFromEnv = ""
	}
	...
	if len(image) == 0 {
		image = imageFromEnv
		if len(image) == 0 {
			runtimeImageInfo := strings.Split(defaultImage, ":")
...
// Parse Dataset MountPoint, currently support 3 scheme
// - pvc: use pvc as data source, e.g. pvc://pvcname[/subpath]
// - local: use hostpath as data source, e.g. local:///host/path
// - cachefs: normal cachefs, no data source, e.g. cachefs:///[subpath]
func ParseSubPathFromMountPoint(mountPoint string) (typ string, pvc string, subpath string, rerr error) {
	cPath := strings.Split(mountPoint, "://")
	if len(cPath) != 2 {
		rerr = fmt.Errorf("MountPoint error, invalid format %s", mountPoint)
	}
	if cPath[0] == "" {
		rerr = fmt.Errorf("MountPoint error: empty scheme %s", mountPoint)
	}
	...
	case SourceTypePvc:
		if cPath[1] == "" {
			typ = ""
			rerr = fmt.Errorf("MountPoint error: missing PVC name %s", mountPoint)
			return
		}
		paths := strings.SplitN(cPath[1], "/", 2)
		if len(paths) == 1 {
			pvc = paths[0]
		} else if len(paths) == 2 {
			pvc = paths[0]
	...
	case SourceTypeLocal, SourceTypeCachefs:
		// When type is local, subpath is path on host.
		subpath = cPath[1]
		if !strings.HasPrefix(subpath, "/") {
			subpath = "/" + cPath[1]
		}
	default:
		rerr = fmt.Errorf("MountPoint error, unsupported scheme %s", typ)
		typ = ""
		return
	}
	return
...
// convert redis maxmemory config string, e.g. 10mb, 1g, to resource.Quantity
func maxMemoryToQuantity(maxMemory string) (resource.Quantity, error) {
	maxMemory = strings.TrimSuffix(strings.ToUpper(maxMemory), "B")
	if strings.LastIndexAny(maxMemory, "KMGTP") > 0 {
		// change strings like "10M" to "10Mi", which resource.ParseQuantity() could parse
		maxMemory = maxMemory + "i"
	}

	return resource.ParseQuantity(maxMemory)
}
