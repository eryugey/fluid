...
func (c *CacheFSEngine) generateDataLoadValueFile(r cruntime.ReconcileRequestContext, object client.Object) (valueFileName string, err error) {
	...
	cacheinfo, value, err := GetCacheInfoFromConfigmap(c.Client, dataload.Spec.Dataset.Name, dataload.Spec.Dataset.Namespace)
	if err != nil {
		return
	}
	...
	dataLoadValue := c.genDataLoadValue(image, cacheinfo, pods, targetDataset, dataload, value)
...
func (c *CacheFSEngine) genDataLoadValue(image string, cacheinfo map[string]string, pods []v1.Pod, targetDataset *datav1alpha1.Dataset, dataload *datav1alpha1.DataLoad, value *CacheFS) *cdataload.DataLoadValue {
	...
	options["importSource"] = value.ImportSource
	if value.Source == "${METAURL}" {
		options["metaurlSecret"] = value.Configs.MetaUrlSecret
		options["metaurlSecretKey"] = value.Configs.MetaUrlSecretKey
	} else {
		options["source"] = value.Source
	}
...
func (c *CacheFSEngine) CheckExistenceOfPath(targetDataload datav1alpha1.DataLoad) (notExist bool, err error) {
	// get mount path
	cacheinfo, _, err := GetCacheInfoFromConfigmap(c.Client, targetDataload.Spec.Dataset.Name, targetDataload.Spec.Dataset.Namespace)
	if err != nil {
		return
	}
