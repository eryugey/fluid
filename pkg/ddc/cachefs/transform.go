...
func (c *CacheFSEngine) transform(runtime *datav1alpha1.CacheFSRuntime) (value *CacheFS, err error) {
	if runtime == nil {
	...
	// allocate ports
	err = c.allocatePorts(runtime, value)
	if err != nil {
		return
	}

	// transform the fuse
	err = c.transformFuse(runtime, dataset, value)
	if err != nil {
		return
	}

	// transform the workers
	err = c.transformWorkers(runtime, dataset, value)
	if err != nil {
		return
	}

	// transform runtime pod metadata
	err = c.transformPodMetadata(runtime, value)
	if err != nil {
		return
	}

	// set the placementMode
	c.transformPlacementMode(dataset, value)
	return
}

func (c *CacheFSEngine) transformWorkers(runtime *datav1alpha1.CacheFSRuntime, dataset *datav1alpha1.Dataset, value *CacheFS) (err error) {
	...
	value.Image, value.ImageTag, value.ImagePullPolicy, err = c.parseCacheFSImage(image, imageTag, imagePullPolicy)
	if err != nil {
		return
	}
	...
	option, err := c.genMountOptions(mount, tieredStoreLevel)
	if err != nil {
		return err
	}
	...
	c.genWorkerMount(value, option)

	// transform resources for worker
	err = c.transformResourcesForWorker(runtime, value)
	if err != nil {
		c.Log.Error(err, "failed to transform resource for worker")
		return
	}

	// transform volumes for worker
	err = c.transformWorkerVolumes(runtime, value)
	if err != nil {
		c.Log.Error(err, "failed to transform volumes for worker")
	}
	// transform cache volumes for worker
	err = c.transformWorkerCacheVolumes(runtime, value)
	if err != nil {
		c.Log.Error(err, "failed to transform cache volumes for worker")
		return err
	}

	// transform import cmd, must be called after transformWorkerVolumes()
	c.genImportCmd(value)
...
// genWorkerMount: generate mount args for worker pods
func (c *CacheFSEngine) genWorkerMount(value *CacheFS, workerOptionMap map[string]string) {
	...
	// start independent cache cluster, fuse and worker use the same cache-group, fuse use no-sharing
	...
	workerOptionMap["backup-meta"] = "0"
	workerOptionMap["cache-group-backsource"] = ""

	mountArgsWorker = []string{common.CacheFSMountPath, value.Source, value.Worker.MountPath, "-o", strings.Join(genArgs(workerOptionMap), ",")}

	value.Worker.Command = strings.Join(mountArgsWorker, " ")
	value.Worker.StatCmd = "stat -c %i " + value.Worker.MountPath
}

// genImportCmd: generate import command for worker pods
func (c *CacheFSEngine) genImportCmd(value *CacheFS) {
	if value.ImportSource == "" {
		return
	}
	value.Configs.ImportCmd = fmt.Sprintf("%s import --validate %s %s", common.CacheFSCliPath, value.Source, value.ImportSource)
}

func (c *CacheFSEngine) transformPlacementMode(dataset *datav1alpha1.Dataset, value *CacheFS) {
...
func (c *CacheFSEngine) transformTolerations(dataset *datav1alpha1.Dataset, value *CacheFS) {
...
func (c *CacheFSEngine) transformPodMetadata(runtime *datav1alpha1.CacheFSRuntime, value *CacheFS) (err error) {
...
func (c *CacheFSEngine) allocatePorts(runtime *datav1alpha1.CacheFSRuntime, value *CacheFS) error {
	...
	if err != nil {
		c.Log.Error(err, "can't get runtime port allocator")
		return err
	}

	allocatedPorts, err := allocator.GetAvailablePorts(expectFusePodNum + expectWorkerPodNum)
	if err != nil {
		c.Log.Error(err, "can't get available ports", "expected port num", expectFusePodNum+expectWorkerPodNum)
		return err
	}
