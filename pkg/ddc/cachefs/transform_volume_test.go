...
func TestTransformWorkerVolumes(t *testing.T) {
	...
	testCases := []testCase{
		...
		},
		{
			...
			mount: datav1alpha1.Mount{
				MountPoint: "pvc://pvcname/subpath",
			},
			expect: &CacheFS{
				ImportSource: DataSourceMountPath + "/subpath",
				Worker: Worker{
					Volumes: []corev1.Volume{
						{
							Name: "pvcname",
		...
		},
		{
			...
			expect: &CacheFS{
				ImportSource: DataSourceMountPath,
				Worker: Worker{
					Volumes: []corev1.Volume{
						{
							Name: "data-source",
	...
	for _, testCase := range testCases {
		...
		got := &CacheFS{}
		err := engine.transformWorkerVolumes(testCase.runtime, got, testCase.mount)
		...
		if !reflect.DeepEqual(got, testCase.expect) {
			t.Errorf("testcase %s\nwant %v\ngot  %v", testCase.name, testCase.expect, got)
		}
...
func TestCacheFSEngine_transformWorkerCacheVolumes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &CacheFSEngine{}
			if err := c.transformWorkerCacheVolumes(tt.args.runtime, tt.args.value); (err != nil) != tt.wantErr {
			...
			if len(tt.args.value.Worker.Volumes) != len(tt.wantVolumes) {
				t.Errorf("%s:\nwant volumes: %v\ngot volumes:  %v", tt.name, tt.wantVolumes, tt.args.value.Worker.Volumes)
			}
			...
			if len(tt.args.value.Worker.VolumeMounts) != len(tt.wantVolumeMounts) {
				t.Errorf("%s:\nwant volumeMounts: %v\ngot volumeMounts:  %v", tt.name, tt.wantVolumeMounts, tt.args.value.Worker.VolumeMounts)
			}
...
func TestCacheFSEngine_transformFuseCacheVolumes(t *testing.T) {
	dir := corev1.HostPathDirectoryOrCreate
	type args struct {
		runtime *datav1alpha1.CacheFSRuntime
		value   *CacheFS
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &CacheFSEngine{}
			if err := c.transformFuseCacheVolumes(tt.args.runtime, tt.args.value); (err != nil) != tt.wantErr {
				t.Errorf("transformFuseCacheVolumes() error = %v, wantErr %v", err, tt.wantErr)
			}

			// compare volumes
			if len(tt.args.value.Fuse.Volumes) != len(tt.wantVolumes) {
				t.Errorf("%s:\nwant volumes: %v\ngot volumes:  %v", tt.name, tt.wantVolumes, tt.args.value.Fuse.Volumes)
			}
			wantVolumeMap := make(map[string]corev1.Volume)
			for _, v := range tt.wantVolumes {
				wantVolumeMap[v.Name] = v
			...
			if len(tt.args.value.Fuse.VolumeMounts) != len(tt.wantVolumeMounts) {
				t.Errorf("%s:\nwant volumeMounts: %v\ngot volumeMounts:  %v", tt.name, tt.wantVolumeMounts, tt.args.value.Fuse.VolumeMounts)
			}
			wantVolumeMountsMap := make(map[string]corev1.VolumeMount)
			for _, v := range tt.wantVolumeMounts {
				wantVolumeMountsMap[v.Name] = v
