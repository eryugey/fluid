...
func Build(id string, ctx cruntime.ReconcileRequestContext) (base.Engine, error) {
	...
	if ctx.Runtime != nil {
		runtime, ok := ctx.Runtime.(*datav1alpha1.CacheFSRuntime)
		if !ok {
			return nil, fmt.Errorf("failed to parse engine %s, can't cast to CacheFSRuntime", ctx.Name)
		}
		engine.runtime = runtime
	} else {
		return nil, fmt.Errorf("failed to parse engine %s, runtime is nil", ctx.Name)
	}
...
func Precheck(client client.Client, key types.NamespacedName) (found bool, err error) {
	var obj datav1alpha1.CacheFSRuntime
