...
func TestCheckRuntimeHealthy(t *testing.T) {
	...
	var cachefsruntimeInputs = []datav1alpha1.CacheFSRuntime{
		{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "hbase",
				Namespace: "fluid",
	...
	engines := []CacheFSEngine{
		{
			...
			namespace: "fluid",
			name:      "hbase",
			runtime:   &cachefsruntimeInputs[0],
		},
		{
			Client:    client,
			Log:       fake.NullLogger(),
			namespace: "fluid",
			name:      "test",
			runtime:   &cachefsruntimeInputs[1],
		},
	}
	...
	for _, test := range testCase {
		...
		if cachefsruntime.Status.WorkerNumberReady != test.expectedRuntimeWorkerNumberReady ||
			cachefsruntime.Status.WorkerNumberAvailable != test.expectedRuntimeWorkerAvailable {
			t.Errorf("fail to update the runtime")
			return
		}
		if cachefsruntime.Status.FuseNumberReady != test.expectedRuntimeFuseNumberReady ||
			cachefsruntime.Status.FuseNumberAvailable != test.expectedRuntimeFuseNumberAvailable {
			t.Errorf("fail to update the runtime")
			return
		}
		_, cond := utils.GetRuntimeCondition(cachefsruntime.Status.Conditions, datav1alpha1.RuntimeWorkersReady)
		if cond == nil {
			t.Errorf("fail to update the condition")
			return
		}
		_, cond = utils.GetRuntimeCondition(cachefsruntime.Status.Conditions, datav1alpha1.RuntimeFusesReady)
		if cond == nil {
			t.Errorf("fail to update the condition")
			return
		}
...
func TestCheckFuseHealthy(t *testing.T) {
	...
	for _, cachefsruntimeInput := range cachefsruntimeInputs {
		testObjs = append(testObjs, cachefsruntimeInput.DeepCopy())
	}
	client := fake.NewFakeClientWithScheme(testScheme, testObjs...)
