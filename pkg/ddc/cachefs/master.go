...
func (c CacheFSEngine) CheckMasterReady() (ready bool, err error) {
	// CacheFS Runtime has no master role
	return true, nil
}
...
func (c CacheFSEngine) SetupMaster() (err error) {
	...
	// 1. Setup
	_, err = kubeclient.GetStatefulSet(c.Client, workerName, c.namespace)
	if err != nil && apierrs.IsNotFound(err) {
		//1. Is not found error
		c.Log.V(1).Info("SetupMaster", "worker", workerName)
		...
		c.Log.V(1).Info("The worker has been set.")
	}
