...
func TestCacheFSEngine_cleanupCache(t *testing.T) {
	...
	testConfigMap := &corev1.ConfigMap{
		...
		Data:       map[string]string{"data": "{\"source\": \"test\", \"citest\": \"true\"}"},
	}
	...
	Convey("Test CleanupCache ", t, func() {
		Convey("cleanup success", func() {
			...
			defer patch1.Reset()
			patch2 := ApplyMethod(reflect.TypeOf(operations.CacheFileUtils{}), "DeleteCacheDirs",
				func(_ operations.CacheFileUtils, cacheDirs []string) error {
					return nil
				})
			defer patch2.Reset()
		...
		Convey("test1", func() {
			...
			defer patch1.Reset()
			patch2 := ApplyMethod(reflect.TypeOf(operations.CacheFileUtils{}), "DeleteCacheDir",
				func(_ operations.CacheFileUtils, cacheDir string) error {
...
func TestCacheFSEngine_getUUID(t *testing.T) {
	...
	ExecCommon := func(a operations.CacheFileUtils, source string) (status string, err error) {
		return CommonStatus, nil
	}
	ExecErr := func(a operations.CacheFileUtils, source string) (status string, err error) {
		return "", errors.New("fail to run the command")
	}
	wrappedUnhookExec := func() {
		err := gohook.UnHook(operations.CacheFileUtils.GetStatus)
		if err != nil {
			t.Fatal(err.Error())
		}
	}
	...
	err := gohook.Hook(operations.CacheFileUtils.GetStatus, ExecErr, nil)
	if err != nil {
		t.Fatal(err.Error())
	}
	...
	err = gohook.Hook(operations.CacheFileUtils.GetStatus, ExecCommon, nil)
	if err != nil {
		t.Fatal(err.Error())
	}
...
func TestCacheFSEngine_cleanAll(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := c.cleanAll(); (err != nil) != tt.wantErr {
...
func TestCacheFSEngine_getCacheDirs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCacheDirs: []string{"/var/cfsCache"},
		},
		{
			name: "test-hostpath",
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &CacheFSEngine{}
			if gotCacheDirs := c.getCacheDirs(tt.args.runtime); !reflect.DeepEqual(gotCacheDirs, tt.wantCacheDirs) {
