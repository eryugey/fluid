...
func (c *CacheFSEngine) transformWorkerVolumes(runtime *datav1alpha1.CacheFSRuntime, value *CacheFS) (err error) {
	if runtime.Spec.ImportDataset != "" {
		sourceInfo := strings.SplitN(runtime.Spec.ImportDataset, ":", 2)
		var dataset string
		var subpath string
		if len(sourceInfo) == 1 {
			dataset = sourceInfo[0]
		} else if len(sourceInfo) == 2 {
			dataset = sourceInfo[0]
			subpath = sourceInfo[1]
		}
