...
func TestCacheFSEngine_ShouldSetupWorkers(t *testing.T) {
	type fields struct {
		name      string
		namespace string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("CacheFSEngine.ShouldSetupWorkers() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotShould != tt.wantShould {
				t.Errorf("CacheFSEngine.ShouldSetupWorkers() = %v, want %v", gotShould, tt.wantShould)
			}
		})
	}
}

func TestCacheFSEngine_SetupWorkers(t *testing.T) {
	...
	}{
		{
			...
			wantedNodeLabels: map[string]map[string]string{
				"test-node": {
					...
					"fluid.io/s-fluid-cachefs":             "true",
					"fluid.io/s-h-cachefs-t-fluid-cachefs": "0B",
					"fluid.io/s-cachefs-fluid-cachefs":     "true",
					"fluid_exclusive":                      "fluid_cachefs",
				},
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = e.SetupWorkers()
			if err != nil {
				t.Errorf("CacheFSEngine.SetupWorkers() error = %v", err)
			}
...
func TestCacheFSEngine_CheckWorkersReady(t *testing.T) {
	type fields struct {
		runtime   *datav1alpha1.CacheFSRuntime
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			runtimeInfo, err := base.BuildRuntimeInfo(tt.fields.name, tt.fields.namespace, "cachefs", datav1alpha1.TieredStore{})
			if err != nil {
				t.Errorf("CacheFSEngine.CheckWorkersReady() error = %v", err)
			}
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("CacheFSEngine.CheckWorkersReady() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotReady != tt.wantReady {
				t.Errorf("CacheFSEngine.CheckWorkersReady() = %v, want %v", gotReady, tt.wantReady)
			}
		})
	}
}

func TestCacheFSEngine_GetWorkerSelectors(t *testing.T) {
	type fields struct {
		name string
	}
	tests := []struct {
	...
	}{
		{
			...
			want: "app=cachefs,release=spark,role=cachefs-worker",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := e.getWorkerSelectors(); got != tt.want {
				t.Errorf("CacheFSEngine.getWorkerSelectors() = %v, want %v", got, tt.want)
			}
		})
	}
}
